...
The random forest model had a precision of 0.869, recall of 0.86, and an accuracy of 0.865, which is decent for only having two classes. But which features were most important in making the decision from which decade the track was? In the plot below we can see that they are:
...
```
The fact that tempo is such an important denoter is not unexpected, as I have already shown the histogram distribution of tempo. The '90s songs show a heavy concentration of tempos at around 90-100BPM, whilst '10s songs have a more equal distribution. 

### Switching up the **tempo**
![Tempogram of Hotline Bling by Drake](hotlinebling_tempogram.jpeg){width=20%} ![Tempogram of Hotline Bling by Drake](hotlinebling_tempogram_cyclic.jpeg){width=20%}
...
In both tempograms it shows a disruption in the horizontal band at around two minutes and thirty seconds, i.e. the bridge. This is caused by the fact that the beat changes at this time stamp. The constant background beat falls off and is replaced with a sparse bass drum and finger snap beat (can this be called half time?) and Drake also changes his rap flow. This makes it more difficult to asses the tempo, which is clearly seen in the both graphs. 
...
***
Coefficient .. looks like the most promising marker distinguishing these genres.
...
souls <-
  get_tidy_audio_analysis("0PV1TFUMTBrDETzW6KQulB") %>% # Change URI.
  compmus_align(bars, segments) %>%                     # Change `bars`
  select(bars) %>%                                      #   in all three
  unnest(bars) %>%                                      #   of these lines.
  mutate(
...
soulsTimbrePlot <- souls %>%
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_classic() +
  labs(x = "", y = "93 'Til Infinity") +
  theme(plot.title = element_text(size=12, hjust = 0.5))

soulsChromaPlot <- souls %>%
  compmus_self_similarity(pitches, "cosine") %>% 
  ggplot(
    aes(
      x = xstart + xduration / 2,
...
figure <- ggarrange(asaprockyTimbrePlot, asaprockyChromaPlot, soulsTimbrePlot, soulsChromaPlot, ncol = 2, nrow = 2, common.legend = TRUE)
...
<iframe src="https://open.spotify.com/embed/track/0PV1TFUMTBrDETzW6KQulB?theme=0" width="100%" height="80" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
...
***
<iframe src="https://open.spotify.com/embed/track/3yjVsUq8psgcHm8SQrZ8i4?theme=0" width="100%" height="80" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
<iframe src="https://open.spotify.com/embed/track/3VmrLy4WZLHDgTXENCIz2p?theme=0" width="100%" height="80" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
...
figure <- ggarrange(bigPoppaPlot, antidotePlot,
...
***
<iframe src="https://open.spotify.com/embed/track/63BcfK6YAzJYeISaTPr6IO?theme=0" width="100%" height="80" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
